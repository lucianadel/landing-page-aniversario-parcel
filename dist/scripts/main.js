...
function handleCheckboxChange() {
    ...
    if (algumPresenteSelecionado) {
        selectedPresentsMessage.textContent = "Voc\xea selecionou um presente. Obrigada!";
...
function clearForm() {
    ...
    for (const checkbox of presentCheckboxes)checkbox.checked = false;
    // Redefinir a mensagem de confirmação
    selectedPresentsMessage.textContent = "Voc\xea n\xe3o selecionou nenhum presente.";
    presentSelection.classList.add("hidden");
...
function getSelecionadosPresents() {
    ...
    presentCheckboxes.forEach((checkbox)=>{
        if (checkbox.checked) presentesSelecionados.push(checkbox.nextElementSibling.textContent);
    });
    return presentesSelecionados;
}
// Adicione um ouvinte de evento para todos os checkboxes
for (const checkbox of presentCheckboxes)checkbox.addEventListener("change", handleCheckboxChange);
...
confirmPresentButton.addEventListener("click", function() {
    ...
    if (name) {
        ...
        const mensagemConfirmacao = `Obrigada pela confirma\xe7\xe3o, ${name}! Voc\xea escolheu os seguintes presentes: ${presentes.join(", ")}.`;
        alert(mensagemConfirmacao);
        clearForm();
    } else alert("Por favor, preencha seu nome.");
});
// Inicializar o cronômetro
setInterval(updateCountdown, 1000);
updateCountdown();

},{}]},["8bDoD","3cYfC"], "3cYfC", "parcelRequire7d08")
